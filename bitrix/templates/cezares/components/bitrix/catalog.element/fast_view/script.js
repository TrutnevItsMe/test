	...
	if (!!this.params.OFFERS && BX.type.isArray(this.params.OFFERS))
	{
		...
		if (!!this.params.OFFER_SELECTED)
		{
			...
			if('offers' in this)
			{
				...
				{
					var sku_id = 0;
					if(window['FAST_VIEW_OID'])
						sku_id = window['FAST_VIEW_OID'];
					if(sku_id)
					{
...
window.JCCatalogElementFast.prototype.ChangeInfo = function()
{
	...
	if (-1 < index)
	{
		...
		if(!!this.offers[index].URL)
		{
			...
			if (arUrl.length > 1) {
				$(this.obProduct).find('.title.hidden a').attr('href', this.offers[index].URL)
		...
		if(arNextOptions['THEME']['CHANGE_TITLE_ITEM'] == 'Y')
		{
			...
			if ($('.fast_view_frame h2 a').length) {
				$('.fast_view_frame h2 a').text(this.offers[index].NAME)
			} else {
				$('.fast_view_frame h2').html($('#fast_view_item .title.hidden').html());
			}
		}
...
/*set store quantity*/
window.JCCatalogElementFast.prototype.setQuantityStore = function(quantity, text, indicators)
{
