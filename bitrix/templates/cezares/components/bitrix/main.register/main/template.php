				...
				<div class="wrap_md">
					<div class="main_info iblock">
						<form method="post" action="<?= POST_FORM_ACTION_URI ?>" name="regform">
							...
							<div class="form-control bg">
								<div class="wrap_md">
									<div class="iblock label_block">
										...
										?>
										<label for="input_SMS_CODE"><?= GetMessage("REGISTER_FIELD_SMS_CODE") ?> <span
													class="star">*</span></label>
										<input id="input_SMS_CODE" size="30" type="text" name="SMS_CODE"
							...
							<div class="but-r">
								<button class="btn btn-default short" type="submit" name="code_submit_button"
										value="<?= GetMessage("main_register_sms_send") ?>"><?= GetMessage("main_register_sms_send") ?></button>
						...
						<form id="registraion-page-form" method="post" action="<?= POST_FORM_ACTION_URI ?>"
							...
							<? foreach ($arFields as $FIELD): ?>
								...
								<? if (($FIELD != "LOGIN" && $arTheme["LOGIN_EQUAL_EMAIL"]["VALUE"] == "Y") || $arTheme["LOGIN_EQUAL_EMAIL"]["VALUE"] != "Y") { ?>
									...
									<label for="input_<?= $FIELD; ?>"><?= $text; ?> <? if ($arResult["REQUIRED_FIELDS_FLAGS"][$FIELD] == "Y"): ?>
											<span class="star">*</span><? endif; ?></label>
								<? } ?>
								...
								<? switch ($FIELD)
								{
									case "PASSWORD":
										?>
										<input size="30" type="password" id="input_<?= $FIELD; ?>"
											   name="REGISTER[<?= $FIELD ?>]" required
											   value="<?= $arResult["VALUES"][$FIELD] ?>" autocomplete="off"
											   class="password <?= (array_key_exists($FIELD, $arResult["ERRORS"])) ? 'error' : '' ?>"/>
									...
									case "CONFIRM_PASSWORD":
										...
										<input size="30" type="password" id="input_<?= $FIELD; ?>"
											   name="REGISTER[<?= $FIELD ?>]" required
											   value="<?= $arResult["VALUES"][$FIELD] ?>" autocomplete="off"
											   class="confirm_password <?= (array_key_exists($FIELD, $arResult["ERRORS"])) ? 'error' : '' ?>"/>
									...
									case "PERSONAL_PHOTO":
									case "WORK_LOGO": ?>
										<input size="30" type="file" id="input_<?= $FIELD; ?>"
											   name="REGISTER_FILES_<?= $FIELD ?>"/>
										<? break;
									case "PERSONAL_NOTES":
									case "WORK_NOTES": ?>
										<textarea cols="30" rows="5" id="input_<?= $FIELD; ?>"
									...
									<? case "PERSONAL_STREET": ?>
										...
										<? break; ?>
									<? case "EMAIL": ?>
										<input size="30" type="email" id="input_<?= $FIELD; ?>"
											   name="REGISTER[<?= $FIELD ?>]" <?= ($arResult["REQUIRED_FIELDS_FLAGS"][$FIELD] == "Y" ? "required" : ""); ?>
											   value="<?= $arResult["VALUES"][$FIELD] ?>" <?= $class ?> id="emails"/>
										<? break; ?>
									<? case "NAME": ?>
										<input size="30" type="text" id="input_<?= $FIELD; ?>"
											   name="REGISTER[<?= $FIELD ?>]" <?= ($arResult["REQUIRED_FIELDS_FLAGS"][$FIELD] == "Y" ? "required" : ""); ?>
											   value="<?= htmlspecialcharsbx($_REQUEST["REGISTER"]["NAME"]) ?>" <?= $class ?>/>
										<? break; ?>
									<? case "PERSONAL_PHONE": ?>
										<input size="30" type="tel" id="input_<?= $FIELD; ?>"
											   name="REGISTER[<?= $FIELD ?>]"
											   class="phone_input <?= (array_key_exists($FIELD, $arResult["ERRORS"])) ? 'error' : '' ?>" <?= ($arResult["REQUIRED_FIELDS_FLAGS"][$FIELD] == "Y" ? "required" : ""); ?>
									...
									default:
										...
										?>
										<input size="30"
											   id="input_<?= $FIELD; ?>" <?= (($FIELD == "LOGIN" && $arTheme["LOGIN_EQUAL_EMAIL"]["VALUE"] == "Y") ? 'type="hidden" value="1"' : 'type="text"'); ?>
											   name="REGISTER[<?= $FIELD ?>]"
											   value="<?= $arResult["VALUES"][$FIELD] ?>"/>
							...
							<? if ($arUFields) { ?>
								<? foreach ($arUFields as $arUField) { ?>
									<div class="r">
										<label><?= $arUField["EDIT_FORM_LABEL"]; ?>
											:<? if ($arUField["MANDATORY"] == "Y"): ?><span
