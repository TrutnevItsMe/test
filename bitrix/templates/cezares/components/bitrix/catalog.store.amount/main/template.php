...
<?if(count($arResult["STORES"]) > 0):?>
	...
	$dbRes = CIBlock::GetList(array(), array('CODE' => 'aspro_next_shops', 'ACTIVE' => 'Y', 'SITE_ID' => SITE_ID));
	...
	<div class="stores_block_wrap">
		...
		<?foreach($arResult["STORES"] as $pid => $arProperty):
			...
			if($arParams['SHOW_EMPTY_STORE'] == 'N' && $amount <= 0)
				$empty_count++;?>
			<div class="stores_block <?=(isset($arProperty["IMAGE_ID"]) && !empty($arProperty["IMAGE_ID"]) ? 'w_image' : 'wo_image')?>" <? echo ($arParams['SHOW_EMPTY_STORE'] == 'N' && $amount <= 0 ? 'style="display: none"' : ''); ?>>
