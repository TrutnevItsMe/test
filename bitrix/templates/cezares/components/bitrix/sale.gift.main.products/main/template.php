	...
	<div id="<?= $injectId ?>" class="bx_sale_gift_main_products common_product wrapper_block">
		...
		if($arResult['HAS_MAIN_PRODUCTS'])
		{
			...
			if(intval($arParams['USE_REGION']) > 0){
				$searchFilter['PROPERTY_LINK_REGION'] = $arParams['USE_REGION'];
				CNext::makeElementFilterInRegion($searchFilter, $arParams['USE_REGION']);
			}
			?>
			<?if(empty($arParams['HIDE_BLOCK_TITLE']) || $arParams['HIDE_BLOCK_TITLE'] !== 'Y'){?>
				<div class="top_block">
					<h4 class="title_block"><? echo ($arParams['BLOCK_TITLE']? htmlspecialcharsbx($arParams['BLOCK_TITLE']) : GetMessage('SLB_TPL_TITLE_GIFT')) ?></h4>
				</div>
			<?}?>
			<?
