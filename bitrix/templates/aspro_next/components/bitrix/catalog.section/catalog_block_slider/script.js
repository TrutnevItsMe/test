...
window.JCCatalogSection.prototype.setPrice = function(change, sku, obPrices, measure, product, offer)
{
	...
	if (!!this.obPrice){
		...
		/*
		strPrice = '<span class="values_wrapper">'+getCurrentPrice(obPrice.DISCOUNT_VALUE, obPrice.CURRENCY, obPrice.PRINT_DISCOUNT_VALUE)+'</span>';
		if(measure){
		...
		{
			$(this.obPriceOld).find('strike').show().html('<span class="values_wrapper">'+getCurrentPrice(obPrice.VALUE, obPrice.CURRENCY, obPrice.PRINT_VALUE)+'</span>');
			if(this.showPercent){
				$(this.obPriceOld).closest('.cost').find('.sale_block').show();
				$(this.obPriceOld).closest('.cost').find('.value').html(obPrice.DISCOUNT_DIFF_PERCENT_RAW);
				$(this.obPriceOld).closest('.cost').find('.text span').html(getCurrentPrice(obPrice.DISCOUNT_DIFF, obPrice.CURRENCY, obPrice.PRINT_DISCOUNT_DIFF));
			}
		}
		else{
