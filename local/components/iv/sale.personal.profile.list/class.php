...
use Bitrix\Main,
	...
	Bitrix\Main\Loader,
    Intervolga\Custom\ORM\SoglasheniyaSKlientamiTable;
...
class PersonalProfileList extends CBitrixComponent
{
	...
	public function executeComponent()
	{
		...
		$dbUserProps = CSaleOrderUserProps::GetList(
            ...
            false,
            false,
            ["NAME", "PERSON_TYPE_ID", "ID", "PERSON_TYPE", "XML_ID"]
		);
		...
		while($arUserProps = $dbUserProps->GetNext())
		{
			...
			$this->arResult["PROFILES"][$arUserProps["ID"]] = $arResultTmp;

            $q = SoglasheniyaSKlientamiTable::query()
                ->where("UF_KONTRAGENT", $arResultTmp["XML_ID"])
                ->setSelect(["*"]);
            $q_res = $q->exec();
            while ($item = $q_res->fetch()) {
                $item["UF_DATA"] = $item["UF_DATA"]->format("d.m.Y");
                $this->arResult["PROFILES"][$arUserProps["ID"]]["DOCS"][] = $item;
            }
            if (!$this->arResult["PROFILES"][$arUserProps["ID"]]["DOCS"]) {
                $this->arResult["PROFILES"][$arUserProps["ID"]]["DOCS"][] = [
                    "UF_NOMER" => static::NON,
                    "UF_DOGOVOR" => static::NON,
                    "UF_OPLATA" => static::NON,
                    "UF_DATA" => static::NON,
                ];
            }
		}
        ...
        $userPropertiesList = CSaleOrderUserPropsValue::GetList(
            array("SORT" => "ASC"),
            array("USER_PROPS_ID" => $ids),
            false,
            false,
            array("ID", "ORDER_PROPS_ID", "VALUE", "SORT", "PROP_TYPE","NAME", "USER_PROPS_ID")
        );
		while ($item = $userPropertiesList->fetch()) {
            $this->arResult["PROFILES"][$item["USER_PROPS_ID"]]["PROPS"][$item["ORDER_PROPS_ID"]] = $item;
        }
