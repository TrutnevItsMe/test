...
(function ()
{
	...
	BX.Sale.OrderAjaxComponent = {
			...
			if (!isLastNode)
			{
				buttons.push(
					BX.create('A', {
						props: {
							href: 'javascript:void(0)',
							className: 'pull-right btn btn-default btn-md'
						},
		...
		createBasketItemContent: function (data)
		{
			...
			if (set.length)
			{
				titleHtml += '<div class="iv-item-composition-8">Состав комплекта:<br/>';
				var conv = this.htmlspecialcharsEx;
				set.forEach(function (el)
				{
					titleHtml += conv(el) + '<br/>';
				});
				titleHtml += '</div>';
			}
						...
						textNode.appendChild(
							BX.create('DIV', {
								...
								children: [BX.create('UL', {
									props: {className: 'bx-scu-itemlist'},
									children: val
								})]
							})
						);
		...
		editActiveRegionBlock: function (activeNodeMode)
		{
			...
			else
			{
				...
				regionNodeCol = BX.create('DIV', {props: {className: 'flex align-items-stretch justify-content-around'}});
		...
		editDeliveryInfo: function (deliveryNode)
		{
			...
			infoList = BX.create('UL', {
				props: {className: 'bx-soa-pp-list'},
				children: [price, period]
			});
				...
				children: [
					...
					BX.create('DIV', {
						...
						children: [
							logoNode,
							BX.create('DIV', {
								props: {className: 'bx-soa-pickup-l-item-name'},
								text: currentStore.TITLE
							}),
							BX.create('DIV', {
								props: {className: 'bx-soa-pickup-l-item-desc'},
								html: html
							})
						]
					}),
		...
		editActivePropsBlock: function (activeNodeMode)
		{
			...
			{
				BX.remove(BX.lastChild(node));
				if (BX.firstChild(this.propsHiddenBlockNode))
				{
				...
				while (group = groupIterator())
				{
					...
					while (property = propsIterator())
					{
						for (i = 0; i < props.length; i++)
							...
							{
								this.getPropertyRowNode(property, node, false);
							}
					}
				}
			}
		...
		editPropsItems: function (propsNode)
		{
			...
			var propsItemsContainer = BX.create('DIV', {props: {className: 'col-sm-12 bx-soa-customer flex align-items-center justify-content-around'}}),
			...
			while (group = groupIterator())
			{
				...
				while (property = propsIterator())
				{
					...
					if (property.getId() !== this.CLIENT_AGREEMENTS_ID)
					{
		...
		alterDateProperty: function (settings, inputText)
		{
			...
			{
				var target = e.target || e.srcElement,
					parentNode = BX.findParent(target, {
						tagName: 'DIV',
		...
		showValidationResult: function (inputs, errors)
		{
			...
			var input0 = BX.type.isElementNode(inputs[0]) ? inputs[0] : inputs[0][0],
				formGroup = BX.findParent(input0, {
					tagName: 'DIV',
					className: 'form-group'
				}),
			...
			for (i = 0; i < inputs.length; i++)
			{
				inputDiv = BX.findParent(inputs[i], {
					tagName: 'DIV',
					className: 'form-group'
				});
		...
		validateLocation: function (input, arProperty, fieldName)
		{
			...
			var parent = BX.findParent(input, {
					tagName: 'DIV',
					className: 'form-group'
				}),
		...
		{
			var propsMapContainer = BX.create('DIV', {
