...
$draftStatus = SaleUtil::getStatusIdByXml(SALE_DRAFT_STATUS_XML_ID);
...
else
{
	...
	if ($_REQUEST["filter_history"] !== 'Y')
	{
		...
		foreach ($arResult['ORDERS'] as $key => $order)
		{
			if ($orderHeaderStatus !== $order['ORDER']['STATUS_ID'] && $arResult['SORT_TYPE'] == 'STATUS')
			{
				...
				?>
				<h1 class="sale-order-title order-status-<?=$orderHeaderStatus?>">
					<?= Loc::getMessage('SPOL_TPL_ORDER_IN_STATUSES') ?> &laquo;<?=htmlspecialcharsbx($arResult['INFO']['STATUS'][$orderHeaderStatus]['NAME'])?>&raquo;
				</h1>
				<?
			}

            $isDraft = $orderHeaderStatus == $draftStatus;
			?>

			<div class="col-md-12 col-sm-12 sale-order-list-container <?=$orderHeaderStatus?>-<?=$order['ORDER']["ID"]?>">
				<div class="row">
								...
								<div class="sale-order-list-inner-row-body">
									...
									if ($payment['PAID'] === 'N' && $payment['IS_CASH'] !== 'Y' && $payment['ACTION_FILE'] !== 'cash')
									{
                                        ...
                                        if ($isDraft){
                                            ?>
                                            <div class="col-md-3 col-sm-4 col-xs-12 sale-order-list-button-container">
                                                <a class="sale-order-list-button change-draft-status" onclick="changeDraftAjax(this, <?=$order['ORDER']["ID"]?>)">
													<?=Loc::getMessage('CHANGE_DRAFT_STATUS')?>
                                                </a>
                                            </div>
                                            <?
										}
	...
	else
	{
		...
		foreach ($arResult['ORDERS'] as $key => $order)
		{
			...
			<div class="col-md-12 col-sm-12 sale-order-list-container order-status-<?=$orderHeaderStatus?>">
				<div class="row">
