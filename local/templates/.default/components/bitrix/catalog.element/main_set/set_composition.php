...
<? if (count($arResult["SET"]["OPTIONAL"]) > 0): ?>
	...
	foreach ($set_group as $arItems){
			...
			<div class="set_item_new" id="set_item_new_<?=$i?>"
												...
												<div class="set-composition set-composition_group_<?=$i?> plus1">
													...
													>
														<span class="set-composition_checkbox">
															<input id="add_input_<?= $ar_SUB_Item['ID'] ?>" data-article="<?= $ar_SUB_Item['ARTICLE'] ?>" data-name="<?= $ar_SUB_Item['NAME'] ?>" data-price="<?= $ar_SUB_Item['PRICE'] ?>" data-img="<?= $ar_SUB_Item['PREVIEW_PICTURE'] ?>" data-group="<?=$i?>" type="checkbox"<?= $ar_SUB_Item['DEFAULT'] ? ' checked' : '' ?>>
														</span>
													...
													<ul class="set_new_ul">
														<?foreach($arResult["MORE_PHOTO"] as $iii2 => $arImage){
															...
															<li id="photo-<?=$iii2?>" <?=(!$iii2 ? 'class="current"' : 'style="display: none;"')?>>
																...
																<?endif;?>
																<?if(!$isEmpty){?>
																	<a href="<?=($viewImgType=="POPUP" ? $arImage["BIG"]["src"] : "javascript:void(0)");?>" <?=($bIsOneImage ? '' : 'data-fancybox-group="item_slider2"')?> class="<?=($viewImgType=="POPUP" ? "popup_link fancy" : "line_link");?>" title="<?=$title;?>">
																		<img width="200px" src="<?=$arImage["SMALL"]["src"]?>" <?=($viewImgType=="MAGNIFIER" ? "class='zoom_picture'" : "");?> <?=($viewImgType=="MAGNIFIER" ? 'data-xoriginal='.$arImage["BIG"]["src"] : "");?> alt="<?=$alt;?>" title="<?=$title;?>"/>
																		<div class="zoom"></div>
																	</a>
																<?}else{?>
				...
				<span  class="epi2">
					<a class="set_group_items_link" data-fancybox data-src="#set_group_<?=$i?>" href="#set_group_<?=$i?>" >Вариантов <?=$ccc?></a>
				</span>
			</div>
